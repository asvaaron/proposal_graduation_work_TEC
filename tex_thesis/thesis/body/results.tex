...
For all comparisons on table \ref{tab:mcnemar_overall_pairs} the obtained p-values are greater than the significance threshold of $0.05$, indicating no statistically significant differences in overall accuracy between the baseline and enhanced head configurations.
...
\end{table}
\noindent
As shown on table \ref{tab:bootstrap_f1_macro}
none of all confidence intervals include zero and all p-values exceed $0.05$, indicating that no statistically significant differences in Macro-F1 were observed across model scales. Similarly, all per-class comparisons shown on tables \ref{tab:bootstrap_f1_class_pairs_base}, \ref{tab:bootstrap_f1_class_pairs_small} and \ref{tab:bootstrap_f1_class_pairs_large} resulted in non-significant differences under the Bonferroni-corrected threshold ($\alpha^* = 0.0071$) indicating that observed performance differences are not statistically significant.
