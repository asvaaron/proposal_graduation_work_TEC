...
The evaluation of the assumptions of the statistical tool used are reported bellow:

\subsection{Main McNemar Results}

Table \ref{tab:mcnemar_overall_pairs} show the pairwise test for the model with and without head changes for (Base/Small/Large) sizes. 

Consider the following null and alterative hypotheses:

\begin{itemize}
\item  $H_0$: P(A correct, B wrong) = P(A wrong, B correct)  $[n_{01} = n_{10}]$
...
\begin{itemize}
\item $n_{01}$: samples missclassified by model A but correctly classified by Model B
...
The similar values of $n_{01}$ and $n_{10}$ across all model sizes suggest that the enhanced classification heads neither systematically improve nor degrade global prediction accuracy. These results confirm that the proposed head modifications preserve the overall error distribution of the models, supporting subsequent analyses focused on class-level performance, macro-averaged F1-score, and efficiency trade-offs.
...
\item $H_1$: The F1 difference is real and not due to sampling variation
\end{itemize}
...
Figures \ref{fig:cm_base}, \ref{fig:cm_small} and  \ref{fig:cm_large} present a pair of confusion matrices corresponding to the three model configuration sizes evaluated with and without modifications to the head classification module. The matrices are displayed side by side to facilitate direct visual comparison under identical evaluation conditions, also a total of 250 epochs were used for displaying the curves. 
