...
\item Evaluate how the proposed modifications to the  head architecture delivers insightful feature representations that explain the model's decision-making process in \gls{FER}.
	
\item Investigate the robustness of the modified Visual Transformer model under adversarial and challenging image conditions, including noise and data perturbations such as occlusions, illumination variations, and subtle facial deformations, as well as fairness-related aspects involving demographic biases (e.g., age, gender, and ethnicity), using the RAF-DB data set to represent \gls{in-the-wild} conditions.
	
\item Explore how the new MLP head architecture improve the model's resilience to an \gls{in-the-wild} data set by employing standard \gls{FER} evaluation metrics like accuracy, f1-score, recall, confusion matrices, accuracy and loss function curves. 
...
\parbox[t]{10cm}{A proposal thesis work comprising the introduction, theorical framework, objectives and hypothesis, which has been previously approved for the development of the thesis} \\
