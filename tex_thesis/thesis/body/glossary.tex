...
\newglossaryentry{PyTorch}{
    name=PyTorch,
    description={An open-source machine learning and deep learning framework primarily developed by Meta Platforms. It provides tools for building, training, and deploying artificial neural networks using tensor-based computations with support for GPU acceleration. PyTorch is widely used in research and industry due to its dynamic computational graph, flexibility, and strong integration with the Python ecosystem.}
}
...
{
    name=FER,
    description={Facial Expression Recognition refers to methods that identify and classify emotions from facial cues.}
}


\newglossaryentry{ViTs}
{	
    name=ViTs,
    description={Term used in the document for Visual Transformers.}
}


\newglossaryentry{MLP Head}
{	
    name=MLP Head,
    description={In this work, the terms classification head and MLP head are used interchangeably to refer to the final feed-forward network following the transformer backbone.}
}
