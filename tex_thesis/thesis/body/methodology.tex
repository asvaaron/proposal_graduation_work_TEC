...
\subsection{Paired Model Comparison Using McNemar’s Test}
...
$$H_0:P(A correct,B wrong)=P(A_{wrong},B_{correct})$$
...
\subsubsection{Bonferroni Correction for Class‑Level Statistical Analysis}
...
To mitigate this issue, a Bonferroni correction is applied. The Bonferroni correction is a conservative multiple‑comparison adjustment that controls the family‑wise error rate (FWER), defined as the probability of making at least one false positive across a family of hypothesis tests.

Formally, given an overall significance level $\alpha$ and $m$ independent hypothesis tests, the Bonferroni‑adjusted significance threshold is defined as:
$$\alpha^* = \frac{\alpha}{m}$$
