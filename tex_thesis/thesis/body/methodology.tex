...
For each configuration, the \gls{POSTER} Visual Transformer was trained using  the \gls{FER} dataset RAF-DB, and the performance was evaluated based on a set of chosen metrics, such \nameref{sub:accuracy}, \nameref{sub:f1-score},  \nameref{sub:recall},  \nameref{sub:precision},  \nameref{sub:confusion-matrix}. Also an individual section with charts for testing/training validation loss \nameref{sub:loss-curves} and \nameref{sub:confusion-matrix}.

The models were trained in a controlled environment, where the only variations between runs were the levels of the factors being tested. The fixed hyperparameters and the hardware used are listed in Tables~\ref{tab:training_hyperparameters} and~\ref{tab:hardware_used}, respectively. This allowed a rigorous comparison of how  model's backbone sizes and head modifications influence the visual transformer performance and generalizations tasks in \gls{FER}.
...
To further analyze the classification behavior of the model, confusion matrices are employed. A confusion matrix provides a detailed breakdown of predicted versus actual class labels, a shown in image \ref{fig:confusion-matrix-poster}, this chart provides a comprehensive assessment of both correct and incorrect predictions across all emotion categories.
...
Training was performed using NVIDIA GPUs with CUDA acceleration. Both local and cloud-based environments were used, including a desktop workstation equipped with two NVIDIA GeForce RTX 3060 GPUs and Google Colab Pro Plus instances with dynamically allocated NVIDIA GPUs, more hardware details are summarized in table~\ref{tab:hardware_used}. All experiments were executed using \gls{PyTorch}, and deterministic CUDA behavior was enforced by fixing random seeds for Python, NumPy, and \gls{PyTorch}.

Each model configuration was trained for 250 epochs under identical hyperparameter settings, including a training batch size of 200, a validation batch size of 32, the Adam and SAM optimizer, and a learning rate of $4 \times 10^{-5}$. Additional information of the hyperparameters can be found in this table \ref{tab:training_hyperparameters}. Early stopping was not applied in order to ensure consistent training dynamics across all experimental runs. Model checkpoints and training logs were stored for post-hoc analysis and metric aggregation.
...
For conducting all of the experiments the dataset Real-world Affective Faces Database (RAF-DB)\footnote{\url{https://www.kaggle.com/datasets/shuvoalok/raf-db-dataset}}\footnote{\url{http://www.whdeng.cn/RAF/model1.html}} was used. This dataset is widely used in the field of Facial Expression Recognition (\gls{FER}).
