...
For each configuration, the POSTER Visual Transformer was trained using a facial expression dataset RAF-DB, and the performance was evaluated based on a chosen metric, such as classification accuracy Section \ref{sub:accurracy} and F1-score Section \ref{sub:f1-score}.
...
A high precision score indicates that the model produces fewer false positives, meaning it is less likely to incorrectly assign an emotion label to a facial expression that does not belong to that category. This is particularly important in FER tasks where certain emotions share visual similarities, leading to potential misclassification (e.g., happiness vs. surprise).

In this research, precision is computed on a per-class basis and aggregated to evaluate the reliability of the modified Visual Transformer’s predictions. Precision analysis complements recall by highlighting trade-offs between sensitivity and prediction confidence, providing a more comprehensive understanding of model performance.
...
A high recall value indicates that the model successfully minimizes false negatives, which is particularly important in FER tasks where under-detection of certain emotions (e.g., fear or disgust) can negatively impact downstream applications such as affective computing or human–computer interaction
...
\item Potential overfitting or underfitting behavior
