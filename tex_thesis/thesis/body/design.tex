...
Although RAF-DB is widely used for in-the-wild FER, it has several limitations:

\begin{itemize}


\item Contains class imbalance across emotion categories

\item Relies on human annotations, which may introduce subjectivity

\item Does not fully represent extreme real-world scenarios such as heavy occlusion or extreme poses
...
This study focuses exclusively on modifications to the classification head of the POSTER architecture. Consequently improvements cannot be attributed to changes in feature extraction. Also, potential gains from modifying attention mechanisms or fusion strategies are not explored

\subsubsection{Computational Constraints}

The depth and number of experimental configurations are constrained by available computational resources. As a result:

\begin{itemize}

\item Extremely deep or large head architectures are not evaluated

\item Training is performed under fixed hardware and time limitations
